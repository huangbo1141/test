...
public class BluetoothDevicesActivity extends CommonActivity {
    ...
    private void sendPinCommandToDevice(String pin){
        try{
            ...
            UIHelper.makeLongToast("Sending "+cmd,AppCache.currentActivity,true);
            if (!AppCache.sabHelper.writeCommand(cmd)){
                UIHelper.makeLongToast("Error in writing command",AppCache.currentActivity,true);
            }else{
                localStorage.setPinCode(pin);
            }
        }catch(Exception ex){
            UIHelper.makeLongToast("Exception: "+ex.getMessage(),AppCache.currentActivity,false);
        }
    }

    private void showPinInputDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle(getString(R.string.device_pin_code));
    ...
    private void onDeviceSelected(final BluetoothDevice device) {
        UIHelper.makeLongToast(getString(R.string.Connection_to) + device.getName(), BluetoothDevicesActivity.this, false);
            ...
            public void run() {
                try {
                    if (!AppCache.sabHelper.setDeviceAndConnect(device)) {
                        UIHelper.msbox("Error", "Unable to start connection to device", BluetoothDevicesActivity.this);
                    }
                } catch (Exception ex) {
                    progressBar.dismiss();
                    UIHelper.msbox("Error", "Error in connect: " + ex.getMessage(), BluetoothDevicesActivity.this);
                }
            }
        };
        thr.start();
    ...
    private void requestEnableBluetooth() {
        UIHelper.makeLongToast(getString(R.string.bluetooth_not_enable_plesse_connect), BluetoothDevicesActivity.this, false);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_ENABLE_BT) {
            bluetoothManager = (BluetoothManager) getSystemService(Context.BLUETOOTH_SERVICE);
            if (mBluetoothAdapter == null || !mBluetoothAdapter.isEnabled()) {
                UIHelper.makeLongToast(getString(R.string.Bluetooth_not_enabled_onActivityResult), BluetoothDevicesActivity.this, false);
            } else {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                    ...
                    UIHelper.makeLongToast(getString(R.string.Location_permission_denied), BluetoothDevicesActivity.this, false);
                }
            }
        } else {
    ...
    private void startScanning() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            UIHelper.makeLongToast("Lollipop or above", BluetoothDevicesActivity.this,true);
            startLollipopLeScanning();
        } else {
            UIHelper.makeLongToast("Below lollipop", BluetoothDevicesActivity.this,true);
    ...
    private void scanningFinished() {
        if (!AppCache.sabHelper.isDeviceConnected()) {
            ...
            if (devId != null) {
                UIHelper.makeLongToast("Old device was: " + devId, AppCache.currentActivity,true);
                devId = devId.toLowerCase();
                for (int a = 0; a < mDevices.size(); a++) {
                    String rDevID = mDevices.get(a).getAddress().toLowerCase();
                    if (rDevID.compareTo(devId.toLowerCase()) == 0) {
                        UIHelper.makeLongToast("Device found in list: " + rDevID, AppCache.currentActivity,true);
    ...
    private ScanCallback lollipopLeCallback = new ScanCallback() {
        ...
        public void onScanFailed(int errorCode) {
            ...
            UIHelper.makeLongToast("onScanFailed: " + String.valueOf(errorCode), BluetoothDevicesActivity.this,false);
            progressBar.dismiss();
        }
    ...
    DefaultSabEventListener sabEventListener = new DefaultSabEventListener() {
        ...
        public void onDeviceConnected(SabBluetoothHelper sab) {
            UIHelper.makeLongToast("BDA: Device connected",AppCache.currentActivity);
