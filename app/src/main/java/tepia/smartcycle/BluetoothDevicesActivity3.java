...
public class BluetoothDevicesActivity3 extends CommonActivity {

    private static final String TAG = BluetoothDevicesActivity3.class.getName();
    ...
    private void onDeviceSelected(final BluetoothDevice device) {
        UIHelper.makeLongToast(getString(R.string.Connection_to) + device.getName(), BluetoothDevicesActivity3.this, false);
            ...
            public void run() {
                try {
                    if (!AppCache.sabHelper.setDeviceAndConnect(device)) {
                        UIHelper.msbox("Error", "Unable to start connection to device", BluetoothDevicesActivity3.this);
                    }
                } catch (Exception ex) {
                    progressBar.dismiss();
                    UIHelper.msbox("Error", "Error in connect: " + ex.getMessage(), BluetoothDevicesActivity3.this);
                }
            }
        };
        thr.start();
    ...
    private void requestEnableBluetooth() {
        UIHelper.makeLongToast(getString(R.string.bluetooth_not_enable_plesse_connect), BluetoothDevicesActivity3.this, false);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_ENABLE_BT) {
            bluetoothManager = (BluetoothManager) getSystemService(Context.BLUETOOTH_SERVICE);
            if (mBluetoothAdapter == null || !mBluetoothAdapter.isEnabled()) {
                UIHelper.makeLongToast(getString(R.string.Bluetooth_not_enabled_onActivityResult), BluetoothDevicesActivity3.this, false);
            } else {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                    ...
                    UIHelper.makeLongToast(getString(R.string.Location_permission_denied), BluetoothDevicesActivity3.this);
                }
            }
        } else {
    ...
    private void startScanning() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            UIHelper.makeLongToast("Lollipop or above", BluetoothDevicesActivity3.this);
            startLollipopLeScanning();
        } else {
            UIHelper.makeLongToast("Below lollipop", BluetoothDevicesActivity3.this);
    ...
    private ScanCallback lollipopLeCallback = new ScanCallback() {
        ...
        public void onScanFailed(int errorCode) {
            ...
            UIHelper.makeLongToast("onScanFailed: " + String.valueOf(errorCode), BluetoothDevicesActivity3.this);
            progressBar.dismiss();
        }
