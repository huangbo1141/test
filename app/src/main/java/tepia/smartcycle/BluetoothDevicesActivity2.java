...
public class BluetoothDevicesActivity2 extends CommonActivity {

    private static final String TAG = BluetoothDevicesActivity2.class.getName();

    private static final int REQUEST_COARSE_LOCATION_PERMISSION = 123;
    private static final int REQUEST_ENABLE_BT = 1245;
    private static final int SCAN_DURATION = 6000;
    ...
    private void onDeviceSelected(final BluetoothDevice device) {
        UIHelper.makeLongToast(getString(R.string.Connection_to) + device.getName(), BluetoothDevicesActivity2.this, false);
            ...
            public void run() {
                try {
                    if (!AppCache.sabHelper.setDeviceAndConnect(device)) {
                        UIHelper.msbox("Error", "Unable to start connection to device", BluetoothDevicesActivity2.this);
                    }
                } catch (Exception ex) {
                    progressBar.dismiss();
                    UIHelper.msbox("Error", "Error in connect: " + ex.getMessage(), BluetoothDevicesActivity2.this);
                }
            }
        };
        thr.start();
    ...
    private void requestEnableBluetooth() {
        UIHelper.makeLongToast(getString(R.string.bluetooth_not_enable_plesse_connect), BluetoothDevicesActivity2.this, false);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_ENABLE_BT) {
            bluetoothManager = (BluetoothManager) getSystemService(Context.BLUETOOTH_SERVICE);
            if (mBluetoothAdapter == null || !mBluetoothAdapter.isEnabled()) {
                UIHelper.makeLongToast(getString(R.string.Bluetooth_not_enabled_onActivityResult), BluetoothDevicesActivity2.this, false);
            } else {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                if (checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                    ...
                    UIHelper.makeLongToast(getString(R.string.Location_permission_denied), BluetoothDevicesActivity2.this, false);
                }
            }
        } else {
    ...
    private void startScanning() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            UIHelper.makeLongToast("Lollipop or above", BluetoothDevicesActivity2.this);
            startLollipopLeScanning();
        } else {
            UIHelper.makeLongToast("Below lollipop", BluetoothDevicesActivity2.this);
    ...
    private ScanCallback lollipopLeCallback = new ScanCallback() {
        ...
        public void onScanFailed(int errorCode) {
            ...
            UIHelper.makeLongToast("onScanFailed: " + String.valueOf(errorCode), BluetoothDevicesActivity2.this);
            progressBar.dismiss();
        }
    ...
    public Action getIndexApiAction() {
        Thing object = new Thing.Builder()
                .setName("BluetoothDevices Page") // TODO: Define a title for the content shown.
